				...
				if (sd.sl_nguonDonDenSearch)
				{
					...
					setTimeout(
							function(){
								...
								{
									if (sd.sl_nguonDonDongChiSearch)
										$("#fetch_sl_nguonDonDongChiSearch").val(sd.sl_nguonDonDongChiSearch);
								...
								if (sd.sl_nguonDonCucVuSearch)
									$("#sl_nguonDonCucVuSearch").val(sd.sl_nguonDonCucVuSearch);
...
// todo: implement quick search function
...
//	hideSaveAndCreateNewButton();
	...
	//Nguồn đơn
	// lanh dao thanh tra chinh phu
	if (sodk.nguonDon == '4' || sodk.nguonDon == 4)
	{
		//Chức vụ (Từ nguồn đơn Lãnh đạo TTCP)
		if (sodk.maChucVuCD && sodk.maChucVuCD != "")
		{
			$("#fetch_sl_nguonDonDongChi").val(sodk.maCoQuanCD);
			$("#fetch_sl_nguonDonChucVu").val(sodk.maChucVuCD);
		}
	}
	...
	//Cơ quan chuyển đơn (Từ nguồn cơ quan khác chuyển)
	if (sodk.nguonDon == '2' || sodk.nguonDon == 2)
	{
		setTimeout(function(){
				if (sodk.maCoQuanCD)
				{
	...
	// cuc vu thuoc thanh tra chinh phu
	if (sodk.nguonDon == '5' || sodk.nguonDon == 5)
	{
		setTimeout(function(){
				if (sodk.maCoQuanCD)
				{
					$("#sl_nguonDonCucVu").val(sodk.maCoQuanCD);
...
function sl_coQuanCD_onChange(id,val){
	...
	}else if(id == 'sl_nguonDonDongChi'){
		$("#txt_coQuanCD").val($('#sl_nguonDonDongChi option:selected').text());
		$("#txt_maCoQuanCD").val(val);
	}else if(id == 'sl_nguonDonCucVu'){
		$("#txt_coQuanCD").val($('#sl_nguonDonChucVu option:selected').text());
		$("#txt_maCoQuanCD").val(val);
	}
}
...
//			$('<option></option>').val(this.id).html(this.name));
	...
	var temp1 = $("#fetch_sl_nguonDonDongChi").val();
	if (temp1 != "")
	{
		$('#sl_nguonDonDongChi').val(temp1);
		$("#fetch_sl_nguonDonDongChi").val("");
	}
	
	$('#sl_nguonDonDongChi').trigger("change");
	return false;
}

//Chức vụ search (Nguồn đơn từ TTCP)
function fetchThamQuyenThuocSearchSuccess(json) {
	$('#sl_nguonDonDongChiSearch').find('option').remove().end();
	...
	var temp1 = $("#fetch_sl_nguonDonDongChiSearch").val();
	if (temp1 != "")
	{
		$('#sl_nguonDonDongChiSearch').val(temp1);
		$("#fetch_sl_nguonDonDongChiSearch").val("");
	}
				   ...
				   function(json){
				   		...
				   		if (filescanList)
					   	{
							...
							var i;
					   		...
					   		{
						   		var filescan = filescanList[i];
						   		var ngayScan;
						   		if (filescan.ngayViet == null)
						   			ngayScan = "";
						   		else
						   			ngayScan = filescan.ngayViet;
					   			
						   		var tomTat;
						   		if (filescan.tomTat == null)
							   		tomTat = "";
						   		else
							   		tomTat = filescan.tomTat;
						   		
						   		html += "<tr id='filescan_tr_" + filescan.id + "'>";
						   			html += '<td style="width:30px;">' + filescan.id + '</td>';
						   			html += '<td style="text-align:left; vertical-align: top;">' + filescan.name + '</td>';
						   			html += '<td style="text-align:center; width:70px; vertical-align: top;">' + ngayScan + '</td>';
						   			html += '<td style="text-align:left; width:125px; vertical-align: top;">' + tomTat + '</td>';
...
//						   				html += '<a href="' + baseUrl + '/upload/filescan/'+ filescan.name + '" target="_blank"><img title="Xem" style="cursor:pointer; vertical-align: middle;" src="images/download.png"></a>';
						   				html += '<a href="javascript:void(0);" onclick="downloadFilescan(' + filescan.id + ')"><img title="Xem" style="cursor:pointer; vertical-align: middle;" src="images/download.png"></a>';
		    	...
		    	"Bạn có chắc muốn xóa file này?",
	    	    {
	    			buttons: {					
						"CÓ": function() {
							$( this ).dialog( "close" );
							doBlockUI();
							var url = baseUrl + "/deleteFilescan?id="+escape(id);
							$.ajax({
								   type: "POST",
								   url: url,
								   dataType: "json", 
...
// PRINT FORM
	...
	function bindKeyShortcutPrintDialogForObject(jquery_obj)
	{
		...
		jquery_obj.bind('keydown', 'alt+2', fncHuyBoPhieuIn);
		return false;
	}
