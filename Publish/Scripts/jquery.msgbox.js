...
(function($) {
  ...
  $.extend({
    MsgBoxObject: {
      defaults    : {
    	  			  unique		  : false,	// unique box? ->>>> them moi
      ...
      show: function(txt, options, callback) {
    	...
    	// unique -> chi cho phep mo 1 cua so popup
        ...
        this.esqueleto.msgbox.queue(this.options.name, $.proxy(function( next ) {
          ...
          if (options.type == 'alert' || options.type == 'info' || options.type == 'error' || options.type == 'confirm')
          {
            $.each(buttons, $.proxy(function(i, button) {
              ...
              {
                this.esqueleto.buttons.append($('<button type="submit">'+button.value+'</button>').bind(
                		'click',
                		$.proxy(function(e) { 
                			// code them moi cho confirm
		                	if (options.type == 'confirm') 
		                		this.close(true); 
		                	else
		                	// end code them moi cho confirm
     ...
     // code them moi   
        // set focus to first button if alert, confirm
        if (options.type == 'alert' || options.type == 'info' || options.type == 'error' || options.type == 'confirm')
        {
        	var btn_arr = this.esqueleto.buttons.find("button");
        	var first_btn = this.esqueleto.buttons.find("button").first();
        	var last_btn = this.esqueleto.buttons.find("button").last();
        	var msgObj = this;
        	btn_arr.bind('keydown', function(e) { 
        	...
        	// last btn tab -> first btn
        	if (btn_arr.length > 0)
        	{
        		last_btn.bind('keydown', function(e) { 
		      		  var keyCode = e.keyCode || e.which; 
		      		  if (keyCode == 9) { 
		      		    e.preventDefault(); 
		      		    // call custom function here
      ...
      close: function(param) {
        ...
        // cdoe them moi de assign confirm result
        if (numberOfOpenedMsgbox > 0)
        	numberOfOpenedMsgbox --;
        
        if (param === true || param === false)
        	this.confirm_result = param;
        // end code them moi
