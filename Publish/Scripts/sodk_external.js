...
//				$("#p1_label").show();
//				$("#p1_content").show();
				$("#p1_label").css("display", "");
				$("#p1_content").css("display", "");
				$("#p1_label").html(nguon_don.p1.title + ":");
				...
				$("#sl_nguonDonChucVu").html(select_html);
				$("#sl_nguonDonChucVu").trigger("change");
			}
			else {
				$("#p1_1_label").css("display", "");
				$("#p1_1_content").css("display", "");
			}

			// p2 visible
			if (nguon_don.p2.visible) {
				$("#p2_label").css("display", "");
				$("#p2_content").css("display", "");
				$("#p2_label").html(nguon_don.p2.title + ":");
			}

			// p3 visible
			if (nguon_don.p3.visible) {
				$("#p3_label").css("display", "");
				$("#p3_content").css("display", "");
				$("#p3_label").html(nguon_don.p3.title + ":");
				if ( ! nguon_don.p1.visible) {
					// get list data from prefetched data
					select_html = "";
					...
					$("#sl_nguonDonCQ").html(select_html);
				}
				$("#sl_nguonDonCQ").trigger("change");
			}
			else {
				$("#p3_1_label").css("display", "");
				$("#p3_1_content").css("display", "");
			}

			// p4 visible
			if (nguon_don.p4.visible) {
				$("#p4_label").css("display", "");
				$("#p4_content").css("display", "");
				$("#p4_label").html(nguon_don.p4.title + ":");
			}
  	    	...
  	    	confirm_show(
  	    	    	"Bạn có chắc muốn xóa lựa chọn này?",
  	        	    {
  	        			buttons: {
  	    					...
  	    					"CÓ": function() {
  	    						doBlockUI();
  	    						window.location.href = (baseUrl == "/" ? baseUrl : baseUrl + "/") + "deleteSodk?id="+escape(id);
  	    					}
  	    				}
  	    	    	}
  	        	);
...
//  				sl_xaPhuongs = $("#sl_xaPhuongs").val();
  	    ...
  	    function open_windown_td(response){        
  			...
  			if (response.status)
  			{
  				...
  				var fullUrl =location.protocol + '//' + location.host + (baseUrl == "/" ? baseUrl : baseUrl + "/") + url;
  	...
  	//// --- 14.
	// 1. fetch paging area when the page is ready
	// 2. Coloring the rows of table, đơn quá hạn, đơn đã xử lý.
		$(document).ready(function(){
			// 1. fetch paging area when the page is ready
			$.ajax({
				type : "POST",
			...
			// 2. Coloring the rows of table, đơn quá hạn, đơn đã xử lý.
			var isQuaHan, isDaXuLy;
			$(".dashboard #table").find("tr").each(function(index) {
				isDaXuLy = $(this).find("td.daXuLyColumn > span").eq(0).html();
				if (isDaXuLy === "1") {
					$(this).addClass("daXuLyRow");
					return 1;
				}

				isQuaHan = $(this).find("td.donQuaHanColumn > span").eq(0).html();
				if (isQuaHan === "1") {
					$(this).addClass("daQuaHanRow");
					return 1;
				}
			});
