...
var ufd_instance_cache = new Array();
...
$.widget(widgetName, {
	...
	_create: function() { //1.8 init
		...
		var sbHeight = this.selectbox[0].offsetHeight;
		...
		this.wrapper.css("height", (sbHeight - 2) + "px" );
	...
	*/
	...
	_populateFromMaster: function() {
		...
		listBuilder.push('<ul>');
//		var temp_1 = this.selectbox.get(0);
//		var options = temp_1.options;
		var options = this.selectbox.get(0).options;
		...
		// cho select index ve cai da chon
		var temp_selected_text = this.input.val();
		var found_element = null;
		this.listItems.each(function(i,elm){
			if ($(this).text() == temp_selected_text)
			{
				found_element = elm;
			}
		})
		this.selectedLi = found_element;
		if (found_element != null)
			this.setActive(found_element);
	...
	setDimensions: function() {
		...
		newSelectWidth = newSelectWidth - 3;
...
/*		console.log(newSelectWidth + " : " + inputWidth + " : " + 
	...
	afterSelect: function(active) {
		...
		if (this.options.updateMasterOnMove)
			this.tryToSetMaster();
...
InfixTrie.prototype.cleanString = function( inStr ) {
	...
	// loai bo dau tieng viet khi search
	inStr = loc_dau_tieng_viet(inStr);
...
function loc_dau_tieng_viet(str) 
{
	str = str.toLowerCase();
