    ...
    load : function( source, selector, config ) {
        ...
        if ( source.constructor === Array ) {
            ...
            $( source ).find( selector ).each( function( i, elem ) {
				...
				// todo: put more attribute here if needed (doan nay tu them vao nhe)
                $.each( 'big title description link layer pdf'.split(' '), function( i, val ) {
                    if ( elem.data(val) ) {
...
this.prependChild( 'info', 'myElement' );
    ...
    */
    ...
    _show : function() {
		...
		// todo: day la doan them moi
		// them moi de hien thi nut print hoac download tuy tinh huong cu the
		var fileName = data.pdf;
		var fileExtension = fileName.substring(fileName.length - 4, fileName.length);
		var showPrintBtn = fileExtension == ".pdf";
		var printBtn = self.$("print"), downloadBtn = self.$("download");
		if (showPrintBtn) {
			printBtn.css("display","block");
			printBtn.data("link",fileName);
			downloadBtn.css("display","none");
		}
		else {
			printBtn.css("display","none");
			downloadBtn.css("display","block");
			downloadBtn.data("link",fileName);
		}
