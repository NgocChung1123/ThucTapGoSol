...
function init_tree(tree_data) {
	...
	if(tree_data.level_creatable)
		level_creatable = tree_data.level_creatable;
		...
		.jstree({
		    ...
		    "contextmenu":
			{
			    ...
			    "items": function (obj) {
			        ...
			        var node = obj;
			        while (hasParentNode) {
			            var aParentNode = $.jstree._focused()._get_parent(node);
			            if (aParentNode === -1)
			                hasParentNode = false;
			            else {
			                node = aParentNode;
			        ...
			        // only show add function for node up to a specific levels
			        if (nodeLevel < level_creatable)
...
// handle delete
		.bind("remove.jstree", function (e, data) {
		    ...
		    var idd = data.rslt.obj.attr("id").replace("node_", "");
		    	            //alert(idd);
		    showDelelePopup(idd);
		    //$("#MainContent_popXoa").show(data);
		    //$("#fade").show();

		    //confirm_show
		    //confirm_show(
			//	"Bạn có chắc muốn xóa cơ quan này?",
			//	{
			//	    buttons: {
			//	        "KHÔNG": function () {
			//	            $.jstree.rollback(data.rlbk);
			...
			//	            //    var idd = o.attr("id").replace("node_", "");
			//	            //    alert(idd);
			//	            //}
			...
			//	                        }
			//	                        else {
			//	                            //alert_error("Cơ quan đã có phòng ban, không thể xóa. Hãy xóa phòng ban trước!");
