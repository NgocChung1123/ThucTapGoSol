...
//				this.hover_node(this._get_prev(o));
				this.deselect_node(this._get_node(o));
...
(function ($) {
	$.jstree.plugin("sort", {
		...
		defaults : function (a, b) {
			//return $.trim(this.get_text(a)).toLowerCase() >  $.trim(this.get_text(b)).toLowerCase() ? 1 : -1;
			return so_sanh_chuoi(this.get_text(a), this.get_text(b));
		},
		_fn : {
			sort : function (obj) {
	...
	$.jstree.plugin("unique", {
				...
				.bind("before.jstree", $.proxy(function (e, data) { 
						...
						if(data.func == "create_node") {
							...
							if(data.args[4] || this._is_loaded(data.args[0])) {
								...
								// code nay them vao de them duoc root node
								if(!p || p === -1) { p = this.get_container(); }
								// end code them vao
		...
		_fn : { 
			_check_unique : function (nms, p, func) {
				...
				// them moi vao de check ky tu chu hoa chu thuong, case insensitive
				var i = 0;
				for (i = 0; i < cnms.length; i++)
					cnms[i] = cnms[i].toLowerCase();
				for (i = 0; i < nms.length; i++)
					nms[i] = nms[i].toLowerCase();
				// end them moi
